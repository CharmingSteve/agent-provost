...
describe("default.conf log schema", function()
  ...
  it("routes access logs to fluent-bit unix socket using json_full", function()
    ...
    assert.is_truthy(conf:match("access_log%s+" .. sock .. ",tag=provost_llm_to_mcp_access%s+json_full"))
    assert.is_truthy(conf:match("access_log%s+" .. sock .. ",tag=provost_mcp_to_api_access%s+json_full"))
  end)
